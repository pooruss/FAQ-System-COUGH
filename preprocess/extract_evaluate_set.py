...
def build_Qq_dict(ars_file_path, scheme):
    ars_dict = {}
    csv_reader = csv.reader(open(ars_file_path,mode='r',encoding='utf-8'))
    csv.field_size_limit(1024 * 1024 * 1024)
    Q_set = set()
    for idx,row in enumerate(csv_reader):
        ...
        # scheme C
            ...
            print("Unrecognized evaluate scheme with {}.".format(scheme))
            break
        if Q not in Q_set:
            ars_dict[Q_index] = {}
...
if __name__ == "__main__":
    ...
    Qq_dict = build_Qq_dict('../data/source/Annotated_Relevance_Set.csv', scheme)
    evaluate_dict = {}
    ...
    np.save('../data/evaluate_set/evaluate_set_scheme_{}_test.npy'.format(scheme), evaluate_dict)
    print (cnt)
    # test
